...
KCSYNTH_DUMMY_CLASS(NSString_KCAdd)
...
+ (NSString *)ServiceStr:(NSString *)url{
    
    NSString *urlHost = @"";
    return [NSString stringWithFormat:@"%@/%@",urlHost,url];

}


+ (NSString *)stringByReversedForPay:(NSString *)str {
    
    if (str.length < 10) return @"";
    
    NSString *upStr = [str substringToIndex:str.length - 10];
    NSString *downStr = [str substringFromIndex:str.length - 10];
    
    NSMutableString *s = [NSMutableString string];
    for (NSUInteger i = upStr.length; i > 0; i--) {
        [s appendString:[upStr substringWithRange:NSMakeRange(i-1, 1)]];
    }
    
    NSString *allstr = [s stringByAppendingString:downStr];
    
    NSData *Base64Data = [[NSData alloc]initWithBase64EncodedString:allstr options:0];
    NSString *base64Decoded = [[NSString alloc]initWithData:Base64Data encoding:NSUTF8StringEncoding];
...
+ (NSString *)getMoneyStringDecimal:(NSString *)string {
    float flStr = string.floatValue;
    string = [NSString stringWithFormat:@"%.0f", flStr];
    
    NSInteger num = flStr/1000;
    while (num) {
        NSString *qitouText = [NSString stringWithFormat:@"%ld", (long)num];
        string = [string stringByReplacingCharactersInRange:NSMakeRange(qitouText.length, 0) withString:@","];
        num = num/1000;
    }
    return string;
}
