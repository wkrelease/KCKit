...
static force_inline KCEncodingNSType KCClassGetNSType(Class cls) {
    if (!cls) return KCEncodingTypeNSUnknown;
    if ([cls isSubclassOfClass:[NSMutableString class]]) return KCEncodingTypeNSMutableString;
    if ([cls isSubclassOfClass:[NSString class]]) return KCEncodingTypeNSString;
    if ([cls isSubclassOfClass:[NSDecimalNumber class]]) return KCEncodingTypeNSDecimalNumber;
    if ([cls isSubclassOfClass:[NSNumber class]]) return KCEncodingTypeNSNumber;
    if ([cls isSubclassOfClass:[NSValue class]]) return KCEncodingTypeNSValue;
    if ([cls isSubclassOfClass:[NSMutableData class]]) return KCEncodingTypeNSMutableData;
    if ([cls isSubclassOfClass:[NSData class]]) return KCEncodingTypeNSData;
    if ([cls isSubclassOfClass:[NSDate class]]) return KCEncodingTypeNSDate;
    if ([cls isSubclassOfClass:[NSURL class]]) return KCEncodingTypeNSURL;
    if ([cls isSubclassOfClass:[NSMutableArray class]]) return KCEncodingTypeNSMutableArray;
    if ([cls isSubclassOfClass:[NSArray class]]) return KCEncodingTypeNSArray;
    if ([cls isSubclassOfClass:[NSMutableDictionary class]]) return KCEncodingTypeNSMutableDictionary;
    if ([cls isSubclassOfClass:[NSDictionary class]]) return KCEncodingTypeNSDictionary;
    if ([cls isSubclassOfClass:[NSMutableSet class]]) return KCEncodingTypeNSMutableSet;
    if ([cls isSubclassOfClass:[NSSet class]]) return KCEncodingTypeNSSet;
...
static force_inline NSDate *KCNSDateFromString(__unsafe_unretained NSString *string) {
...
#define kParserNum 34
    ...
    dispatch_once(&onceToken, ^{
        ...
        
        {
            ...
            NSDateFormatter *formatter = [NSDateFormatter new];
            formatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"];
            formatter.dateFormat = @"EEE MMM dd HH:mm:ss Z KCKC";
            
            NSDateFormatter *formatter2 = [NSDateFormatter new];
            formatter2.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"];
            formatter2.dateFormat = @"EEE MMM dd HH:mm:ss.SSS Z KCKC";
...
static force_inline id KCValueForMultiKeys(__unsafe_unretained NSDictionary *dic, __unsafe_unretained NSArray *multiKeys) {
    id value = nil;
    for (NSString *key in multiKeys) {
        if ([key isKindOfClass:[NSString class]]) {
            ...
            value = KCValueForKeyPath(dic, (NSArray *)key);
            if (value) break;
        }
    }
    return value;
...
@interface _KCModelPropertyMeta : NSObject {
    ...
    KCEncodingType _type;        ///< property's type
    KCEncodingNSType _nsType;    ///< property's Foundation type
    ...
    KCClassPropertyInfo *_info;  ///< property's info
    _KCModelPropertyMeta *_next; ///< next meta if there are multiple properties mapped to the same key.
}
@end
...
+ (instancetype)metaWithClassInfo:(KCClassInfo *)classInfo propertyInfo:(KCClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    if ((meta->_type & KCEncodingTypeMask) == KCEncodingTypeStruct) {
        /*
    ...
    } else if (meta->_cls && meta->_nsType == KCEncodingTypeNSUnknown) {
...
@interface _KCModelMeta : NSObject {
    @package
    KCClassInfo *_classInfo;
    /// Key:mapped key and key path, Value:_KCModelPropertyMeta.
    NSDictionary *_mapper;
    /// Array<_KCModelPropertyMeta>, all property meta of this model.
    NSArray *_allPropertyMetas;
    /// Array<_KCModelPropertyMeta>, property meta which is mapped to a key path.
    NSArray *_keyPathPropertyMetas;
    /// Array<_KCModelPropertyMeta>, property meta which is mapped to multi keys.
...
/**
 ...
 @param _propertyMeta should not be nil, _KCModelPropertyMeta.
...
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ...
    if (propertyMeta->_mappedToKeyArray) {
        value = KCValueForMultiKeys(dictionary, propertyMeta->_mappedToKeyArray);
...
static NSString *ModelDescription(NSObject *model) {
    ...
    switch (modelMeta->_nsType) {
        case KCEncodingTypeNSString: case KCEncodingTypeNSMutableString: {
            return [NSString stringWithFormat:@"\"%@\"",model];
        }
            
        case KCEncodingTypeNSValue:
        case KCEncodingTypeNSData: case KCEncodingTypeNSMutableData: {
        ...
        case KCEncodingTypeNSSet: case KCEncodingTypeNSMutableSet: {
            model = ((NSSet *)model).allObjects;
        } // no break
            
        case KCEncodingTypeNSArray: case KCEncodingTypeNSMutableArray: {
        ...
        case KCEncodingTypeNSDictionary: case KCEncodingTypeNSMutableDictionary: {
        ...
        default: {
            ...
            NSArray *properties = [modelMeta->_allPropertyMetas
                                   sortedArrayUsingComparator:^NSComparisonResult(_KCModelPropertyMeta *p1, _KCModelPropertyMeta *p2) {
                                       return [p1->_name compare:p2->_name];
                                   }];
            
            [desc appendFormat:@" {\n"];
            for (NSUInteger i = 0, max = properties.count; i < max; i++) {
                _KCModelPropertyMeta *property = properties[i];
                NSString *propertyDesc;
                if (property->_isCNumber) {
                    NSNumber *num = ModelCreateNumberFromProperty(model, property);
                    propertyDesc = num.stringValue;
                } else {
                    switch (property->_type & KCEncodingTypeMask) {
                        case KCEncodingTypeObject: {
                        ...
                        case KCEncodingTypeCArray: case KCEncodingTypeCString: case KCEncodingTypePointer: {
                            void *pointer = ((void* (*)(id, SEL))(void *) objc_msgSend)((id)model, property->_getter);
                            propertyDesc = [NSString stringWithFormat:@"%p",pointer];
                        } break;
                        case KCEncodingTypeStruct: case KCEncodingTypeUnion: {
...
+ (NSDictionary *)_KC_dictionaryWithJSON:(id)json {
...
+ (instancetype)modelWithJSON:(id)json {
    NSDictionary *dic = [self _KC_dictionaryWithJSON:json];
...
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _KCModelMeta *modelMeta = [_KCModelMeta metaWithClass:cls];
...
- (BOOL)modelSetWithJSON:(id)json {
    NSDictionary *dic = [NSObject _KC_dictionaryWithJSON:json];
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        dic = [((id<KCModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]]) return NO;
    }
