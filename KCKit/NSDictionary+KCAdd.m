...
KCSYNTH_DUMMY_CLASS(NSDictionary_KCAdd)

@interface _KCXMLDictionaryParser : NSObject<NSXMLParserDelegate>

@end

@implementation _KCXMLDictionaryParser{
...
@implementation NSDictionary (KCAdd)
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _KCXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_KCXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_KCXMLDictionaryParser alloc] initWithData:xml];
    }
...
@implementation NSMutableDictionary (KCAdd)
